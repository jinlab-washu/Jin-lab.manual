...
def format_gnomad_mt(chrom,out_dir="/gpfs/ycga/scratch60/kahle/sp2349/datasets/gnomad/"):
	...
	combined = combined.annotate(exome_rsid = combined.info.rsid)
	combined = combined.annotate(exome_qual = combined.info.qual)
	combined = combined.annotate(exome_filters = combined.info.filters)
	combined = combined.annotate(genome_rsid = combined.info_1.rsid)
    combined = combined.annotate(genome_qual = combined.info_1.qual)
    combined = combined.annotate(genome_filters = combined.info_1.filters)	

	# Drop info and info_1 fields as no longer necessary
	combined = combined.drop(info)
